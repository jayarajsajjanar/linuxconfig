...
6. To configure the firewall,
	...
	2. Everything is set to `deny` using `sudo ufw default deny incoming`. Only the ones that are needed are allowed. This 
	provides additional security to the server.
...
8. To install and configure PostgreSQL:
	...
	3. (step 9 of the main listing i.e. configuring git and cloning was done before this)New user for postgres - `catalog`. New database - `catalog`.
